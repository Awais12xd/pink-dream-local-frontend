...
const EditProductPage = dynamic(() => import('./components/EditProduct'), { ssr: false });
const ViewProducts = dynamic(() => import('./components/ViewProducts'), { ssr: false });
const ProductDetailsPage = dynamic(() => import('./components/ProductDetails'), { ssr: false });
const AdminOrders = dynamic(() => import('./components/AdminOrders'), { ssr: false });
const AdminLogin = dynamic(() => import('./components/AdminLogin'), { ssr: false });
const Promocodemanager = dynamic(() => import('./components/Promocodemanager'), { ssr: false });
const CategoriesPage = dynamic(() => import('./components/Categoriespage'), { ssr: false });
const BlogCategoriesPage = dynamic(() => import('./components/Blog-Componenets/BlogCategoriesPage'), { ssr: false });
const AddBlogPage = dynamic(() => import('./components/Blog-Componenets/AddBlogPost'), { ssr: false });
const ViewBlogs = dynamic(() => import('./components/Blog-Componenets/ViewBlogs'), { ssr: false });
const BlogDetailsPage = dynamic(() => import('./components/Blog-Componenets/BlogDetailsPage'), { ssr: false });
const EditBlogPage = dynamic(() => import('./components/Blog-Componenets/EditBlogPage'), { ssr: false });
...
const AdminPanel = () => {
  ...
  const handleViewBlog = useCallback((blog) => {
    setSelectedBlogForDetails(blog);
    setActiveTab('blog-details');
    setSidebarOpen(false);
  }, []);

  // Handle delete product
  const handleDeleteBlog = useCallback(async (blog) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (activeTab === 'blogs') {
            ...
            setActiveTab('blogs');
            setSelectedBlogForDetails(null);
          }
        } else {
  ...
  const handleEditFromBlogDetails = useCallback((blog) => {
    const blogToEdit = blog || selectedBlogForDetails;
    setSelectedBlogForEdit(blogToEdit);
    setActiveTab('edit-blog');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-pink-50/20 to-rose-50/30">
      ...
      <div className={`fixed inset-y-0 left-0 z-40 bg-white shadow-2xl border-r border-gray-200 transform transition-all duration-300 ease-in-out ${sidebarCollapsed ? 'w-20' : 'w-72'
        ...
        <div className="flex flex-col h-full">
           ...
           100vw"
            />
              ...
              {!sidebarCollapsed && (
                <div className="flex-1 min-w-0">
                  <h1
                    className="text-xl font-bold text-gray-800 truncate cursor-pointer hover:text-pink-600 transition-colors"
                    onClick={openShop}
            ...
            {/* {!sidebarCollapsed && (
              <div className="mt-2 flex items-center gap-1 text-xs text-pink-600">
                ...
                <span className="italic">Click title to view shop</span>
              </div>
            )} */}
          </div>
          ...
          <nav className="flex-1 overflow-y-auto px-4 py-6 space-y-1">
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group ${activeTab === 'dashboard'
            ...
            <div className={sidebarCollapsed ? '' : 'space-y-1'}>
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group ${['products', 'add-product', 'edit-product', 'product-details'].includes(activeTab)
                  ? 'bg-gradient-to-r from-pink-500 to-rose-600 text-white shadow-lg shadow-pink-200'
                  : 'text-gray-700 hover:bg-gray-100'
              ...
              {!sidebarCollapsed && ecommerceOpen && (
                ...
                >
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all duration-200 ${activeTab === 'products'
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all duration-200 ${activeTab === 'add-product'
                      ? 'bg-pink-50 text-pink-700 font-semibold'
                      : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                      }`}
                  >
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all duration-200 ${activeTab === 'categories'
                      ? 'bg-pink-50 text-pink-700 font-semibold'
                      : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                      }`}
                  >
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group ${activeTab === 'orders'
                ? 'bg-gradient-to-r from-pink-500 to-rose-600 text-white shadow-lg shadow-pink-200'
                : 'text-gray-700 hover:bg-gray-100'
                } ${sidebarCollapsed ? 'justify-center' : ''}`}
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group ${activeTab === 'promo-codes'
                ? 'bg-gradient-to-r from-pink-500 to-rose-600 text-white shadow-lg shadow-pink-200'
                : 'text-gray-700 hover:bg-gray-100'
                } ${sidebarCollapsed ? 'justify-center' : ''}`}
            ...
            <div className={sidebarCollapsed ? '' : 'space-y-1'}>
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group ${['blogs', 'add-blog', 'edit-blog', 'blog-details'].includes(activeTab)
                  ? 'bg-gradient-to-r from-pink-500 to-rose-600 text-white shadow-lg shadow-pink-200'
                  : 'text-gray-700 hover:bg-gray-100'
                  } ${sidebarCollapsed ? 'justify-center' : 'justify-between'}`}
                title={sidebarCollapsed ? 'E-commerce' : ''}
              >
                <div className="flex items-center gap-3">
                  <FileText className={`w-5 h-5 flex-shrink-0 ${['blogs', 'add-blog', 'edit-blog', 'blog-details'].includes(activeTab) ? '' : 'group-hover:scale-110'} transition-transform`} />
                  {!sidebarCollapsed && <span className="font-medium">Blog Post</span>}
                </div>
                {!sidebarCollapsed && (
                  blogOpen ? (
                    <ChevronUp className="w-4 h-4 flex-shrink-0" />
              ...
              {!sidebarCollapsed && blogOpen && (
                ...
                >
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all duration-200 ${activeTab === 'blogs'
                      ? 'bg-pink-50 text-pink-700 font-semibold'
                      : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                      }`}
                  >
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all duration-200 ${activeTab === 'add-blog'
                      ? 'bg-pink-50 text-pink-700 font-semibold'
                      : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                      }`}
                  >
                  ...
                  <button
                    onClick={() => {
                      setActiveTab('blog-categories');
                      setSidebarOpen(false);
                    }}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all duration-200 ${activeTab === 'blog-categories'
                      ? 'bg-pink-50 text-pink-700 font-semibold'
                      : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                      }`}
                  >
          ...
          <div className="border-t border-gray-100 p-4 space-y-2 bg-gray-50">
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-red-600 hover:bg-red-50 transition-all duration-200 group ${sidebarCollapsed ? 'justify-center' : ''
                }`}
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-gradient-to-r from-gray-100 to-gray-200 text-gray-700 hover:from-gray-200 hover:to-gray-300 transition-all duration-200 ${sidebarCollapsed ? 'justify-center' : 'justify-between'
                }`}
      ...
      <div className={`transition-all duration-300 min-h-screen bg-gradient-to-br from-gray-50 via-pink-50/20 to-rose-50/30 ${sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-72'}`}>
        ...
        <main className="p-2 sm:p-6">
          ...
          {activeTab === 'blog-details' && (
            selectedBlogForDetails ? (
              <BlogDetailsPage
                blogId={selectedBlogForDetails._id || selectedBlogForDetails.id}
                onEdit={handleEditFromBlogDetails}
                onDelete={() => handleDeleteBlog(selectedBlogForDetails)}
                onBack={handleBackFromBlogDetails}
              />
            ) : (
              <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-pink-100">
                <h3 className="text-lg font-bold bg-gradient-to-r from-pink-600 to-rose-600 bg-clip-text text-transparent mb-4">Blog Details</h3>
                <p className="text-gray-600 mb-4">
                  Select a Blog from the Blog list to view detailed information.
                </p>
                <button
          ...
          {activeTab === 'edit-blog' && (
            selectedBlogForEdit ? (
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-pink-100">
                <h3 className="text-lg font-bold bg-gradient-to-r from-pink-600 to-rose-600 bg-clip-text text-transparent mb-4">Edit Blog</h3>
                <p className="text-gray-600 mb-4">
                  Select a blog from the blogs list to edit.
                </p>
                <button
                  onClick={() => setActiveTab('blogs')}
                  className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-pink-500 to-rose-500 text-white rounded-lg hover:from-pink-600 hover:to-rose-600 transition-all shadow-md hover:shadow-lg"
